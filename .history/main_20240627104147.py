...
def fetch_prices(query):
    ...
    for result in data.get('shopping_results', []):
        prices.append({
            ...
            'price': result['price'],
            'link': result['link'],
            'image': result['thumbnail'] if 'thumbnail' in result else ''
        })
    return prices
...
def track_price():
    ...
    tracked_products = PriceHistory.query.distinct(PriceHistory.product).all()
    return render_template('track.html', tracked_products=tracked_products)
